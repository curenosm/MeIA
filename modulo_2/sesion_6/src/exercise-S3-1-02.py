#!/usr/bin/python3
#   Example S3-1-02: Associative Domain Adaptation (ADA)
#   Commands to execute this code:
#   python3.8 exercise-S3-1-02.py --dir_resume outputs --seed 223
...
def losocv(X, Y, subjects, args):
    ...
    for foldNum, fold in enumerate(fold_pairs):
        ...
        # [OPTIMIZERS]
        optimizer_g = optim.RMSprop(network_f.parameters(), lr=args.lr, weight_decay=0.001)
        optimizer_f = optim.RMSprop(network_h.parameters(), lr=args.lr, weight_decay=0.001)

        # [Classification loss]
        criterion = nn.CrossEntropyLoss().cuda()

        # associative loss
        association = WalkerVisitLosses().cuda()
        ...
        for iter_num in range(0, args.max_iterations + 1):
            ...
            # [ASSOCIATIVE LOSS]
            walker_loss, visit_loss = association(outputs_source, outputs_target, labels_source)
            associative_loss = 1.0 * walker_loss + 0.6 * visit_loss

            # [PRELIMINAR TOTAL LOSS]
            total_loss = classifier_loss + associative_loss

            # Reset gradients
            optimizer_g.zero_grad()  # feature extractor
            optimizer_f.zero_grad()  # classifier
